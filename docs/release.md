...
- **Apprentice ledger** at `~/.rebotica/ledger.duckdb` (DuckDB, bundled). `rbtc run *` writes `run_started` and `run_completed` events; `rbtc score` writes `prime_disposition`. The schema, payload shapes, and derived views (`v_per_model_stats`, `v_per_envelope_stats`, `v_disposition_breakdown`) are documented in [output-contract.md](output-contract.md#apprentice-ledger). Ledger writes are best-effort: a write failure logs a warning but does not abort the command. A pre-v0.3 SQLite ledger (`ledger.db`) is migrated forward to DuckDB on first open (original kept as `ledger.db.sqlite-backup`). The `hallucination_rate` field on `run_completed` is populated by issue #51 in a follow-up.
- **Ledger backend is DuckDB, not SQLite.** v0.3 moves the apprentice ledger to DuckDB to go straight to the columnar engine that the routing/retrospective/federated-benchmark analytics want, rather than swapping later. The binary and cold-build cost grew (bundled DuckDB), an accepted trade now that the MCP server is in and the analytics surface is the point. **After upgrading, reinstall the `rbtc` binary on your `PATH`** (`cargo install --path crates/rebotica-cli` or your usual install) so every invocation uses the DuckDB build — a stale SQLite-era binary left on `PATH` would keep writing the old `ledger.db` alongside the new `ledger.duckdb`.
...
- **MCP `submit_feedback` tool.** Prime can now file product feedback about Rebotica itself (a bad apprentice result, a confusing tool response, a missing capability) from within an MCP session, instead of silently working around a shortcoming. It writes a structured comment card under `~/.rebotica/comment-cards/pending/`; when GitHub submission consent is enabled (`rbtc comment-card consent --allow-github`), it also files a labeled GitHub issue via `gh`, mirroring `rbtc comment-card submit`. Pass `run_id` to tie feedback to a specific delegated run. The comment-card core (model, storage, consent, submit) moved from the CLI into a shared `rebotica-runlog::feedback` module so the CLI and MCP share one implementation.
