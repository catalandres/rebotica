...
- **`local-model-delegation` skill rewritten for MCP-first invocation.** Installed by `rbtc install claude` (into `.claude/skills/`) and `rbtc install codex` (into `.agents/skills/`). Prime is instructed to call `mcp__rebotica__review_diff` / `propose_tests` / `explain_files` / `health_check` before doing the equivalent work itself; the existing `rbtc run *` CLI verbs remain as a documented fallback. The skill ships two snippets users merge into `.claude/settings.json`: `claude-settings-snippet.json` registers the MCP server (so Claude Code launches `rbtc mcp serve` on session start), and `hooks/claude-settings-snippet.json` wires the included `hooks/post-tool-use.sh` to record a placeholder `unscored` disposition after every `mcp__rebotica__*` tool call (Prime can later upgrade with explicit `rbtc score RUN_ID --disposition <value>`). The hook requires `jq` on PATH and soft-fails if anything is missing.
